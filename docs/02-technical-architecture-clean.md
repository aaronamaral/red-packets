...
Wallet interactions are handled by Wagmi v2 + Viem.

**External Services**

| Service | Role | Details |
|---------|------|---------|
| Twitter/X | Authentication and anti-bot | OAuth 2.0 (NextAuth v5), profile data (account age, followers), bearer token (anti-bot) |
| Neon Postgres | Persistent storage | Tables: packets, claims, rate_limits. UUID-to-packet_id mapping. |
| Base Blockchain | Onchain state and fund custody | RedPacket.sol (createPacket, claim, refund), USDC (ERC-20), EIP-712 verification |
| Edge Middleware | Request filtering | Geo-blocking for OFAC sanctioned countries |
...
**Step 1:** Claim URL received (e.g., `/claim/abc123-def456-...`).

**Step 2:** API route extracts UUID from URL path.

**Step 3:** Database lookup: `SELECT packet_id FROM packets WHERE id = 'abc123-def456-...'`.

**Step 4:** Use the returned `packet_id` (integer) for all onchain contract calls.
...
**Participants:** User (Browser), API Route, Blockchain, Database

**Step 1:** User approves USDC spending via wallet transaction. Blockchain executes ERC-20 approve.

**Step 2:** User calls `createPacket` via wallet transaction. Blockchain executes `createPacket()` and emits `PacketCreated` event.

**Step 3:** User receives transaction receipt and events from blockchain.

**Step 4:** User sends `POST /api/packets` with `{ packetId, address, txHash }` to the API route.

**Step 5:** API route inserts a row into the `packets` table in the database.

**Step 6:** Database returns the auto-generated `id` (UUID).

**Step 7:** API route responds to the user with `{ packetId, uuid }`.

**Step 8:** Frontend renders the share card with the UUID-based link.

### Claim Flow

**Participants:** User (Browser), API Route, Blockchain, Database

**Step 1:** User sends `GET /api/packets/[uuid]` to the API route.

**Step 2:** API route resolves UUID to `packet_id` via database lookup (`SELECT packet_id`).

**Step 3:** API route reads onchain state via `readContract(packets)` on the blockchain.

**Step 4:** API route returns packet info to the user.

**Step 5:** User sends `POST /claim { address }` to the API route.

**Step 6:** API route resolves UUID to `packet_id` via database lookup.

**Step 7:** API route reads onchain state to verify: packet exists, not expired, not full.

**Step 8:** API route runs anti-bot checks against the database: account age, followers, duplicate check, rate limit.

**Step 9:** API route performs atomic claim slot reservation: `INSERT INTO claims ... ON CONFLICT DO NOTHING RETURNING id`.

**Step 10:** API route inserts a rate limit record into the database.

**Step 11:** API route signs the claim using EIP-712 and returns `{ signature, nonce, twitterUserId }` to the user.

**Step 12:** User submits `claim(packetId, twitterUserId, nonce, signature)` via wallet transaction. The blockchain contract executes: verify EIP-712 signature, check `hasClaimed`, check `twitterClaimed`, calculate amount, transfer USDC, emit `PacketClaimed`.

**Step 13:** User receives transaction receipt and events from blockchain.

**Step 14:** User sends `POST /confirm { txHash }` to the API route.

**Step 15:** API route calls `getTransactionReceipt` on the blockchain to verify status, parse the event, and read the claimed amount.

**Step 16:** API route updates the claim record in the database with the verified amount and transaction hash.

**Step 17:** API route returns `{ amount }` to the user.

### Refund Flow

**Participants:** User (Browser), API Route, Blockchain

**Step 1:** User submits `refund(packetId)` via wallet transaction.

**Step 2:** Blockchain contract executes: verify caller is the creator and packet is not already refunded, set `refunded = true`, transfer remaining USDC to creator, emit `PacketRefunded`.

**Step 3:** User receives transaction receipt and event from blockchain.
...
**Step 1 -- Template.** Source file: `/public/images/card-template.png`. Dimensions: 1200 x 675 pixels. Designed in Figma with Coinbase/Base branding, red envelope imagery. Includes a placeholder circle for the creator's avatar and a placeholder position for the creator's handle.

**Step 2 -- API Route.** Endpoint: `/api/og/[uuid]`. Runtime: Edge (next/og ImageResponse). Fetches the creator's Twitter handle and avatar URL from Postgres. Upgrades avatar URL from `_normal` to `_400x400` resolution.

**Step 3 -- Compositing.** Layers are combined as follows:

| Layer | Content | Size / Style | Position |
|-------|---------|-------------|----------|
| Base | Template PNG | Full 1200x675 | Full canvas |
| Avatar | Creator's Twitter PFP | 310x310 pixels, circular (borderRadius: 50%) | Centered at (772, 338) |
| Handle | Creator's Twitter handle | 36px sans-serif, white | (277, 365) |

**Step 4 -- Output.** 1200 x 675 PNG served as the `og:image` and `twitter:image` meta tag. Twitter card type: `summary_large_image`.
...
|-------|----------|---------|
| 1 | SessionProvider | NextAuth -- Twitter OAuth session |
| 2 | WagmiProvider | Wallet connection, chain config |
| 3 | QueryClientProvider | TanStack React Query |
| 4 | OnchainKitProvider | Base chain, CDP API key |
| 5 | Layout | Geist fonts, dark theme, global CSS |

**Page Structure**

| Page | Route | Children |
|------|-------|----------|
| Landing Page | `/` | Header, Hero Section, How It Works (4 gold-gradient cards), PacketCard (mode="demo"), Lanterns (10 CSS lanterns, 4 shapes, varied sizes), Fire Horse (PNG with CSS mask gold tint + glow pulse), Footer ("Powered by Base") |
| Create Page | `/create` | Auth gate (requires Twitter session), Wallet gate (requires connected wallet), Form (amount input, recipients input 1-200, split type selector, expiry selector), StepIndicator (approve -> create -> finalize), Success state with PacketCard (mode="preview") and ShareCta |
| Claim Page | `/claim/[uuid]` | 13 states (loading, sealed, auth_required, connecting_wallet, ready, claiming, opening, revealed, already_claimed, expired, fully_claimed, auth_failed, error), PacketCard (mode="claim"), ClaimResult (count-up animation + confetti), ShareCta |
| My Packets | `/my-packets` | PacketCard grid (mode="preview", 4 columns), per-packet status (claimed count / total with progress bar, expiry status), share buttons (X + copy link), withdraw button (gold, shows remaining USDC amount) |

**Header Children:** Logo ("Red Packets"), Link to /my-packets, TwitterSignIn (sign-in / sign-out button), WalletWrapper (connect / disconnect wallet)
